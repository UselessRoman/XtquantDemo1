...
if __name__ == "__main__":
    ...
    print("\n【步骤1】下载历史数据")
    print("-" * 60)
    framework.download_data(stock_id, "1d", start_date, end_date)
    
    # 2. 获取本地数据
    print("\n【步骤2】获取本地数据")
    print("-" * 60)
    data = framework.get_data(stock_id, "1d", start_date, end_date)
    if data is not None:
        print(f"[成功] 成功获取数据，共 {len(data)} 条记录")
    
    # 3. 分析数据
    print("\n【步骤3】分析数据并绘制图表")
    print("-" * 60)
    framework.analyze_data(stock_id, "1d", start_date, end_date)
    ...
    result1 = framework.run_backtest(stock_id, macd_strategy, "1d", start_date, end_date)
    
    # 策略2: 均线策略
    print("\n策略2: 均线策略 (MA5上穿MA10)")
    ma_strategy = MAStrategy(use_multiple_ma=True)
    result2 = framework.run_backtest(stock_id, ma_strategy, "1d", start_date, end_date)
    
    # 策略3: 组合策略
    print("\n策略3: 组合策略 (MACD + 均线)")
    combined_strategy = CombinedStrategy(
        strategies=[MACDStrategy(), MAStrategy(use_multiple_ma=True)],
        vote_threshold=1
    )
    result3 = framework.run_backtest(stock_id, combined_strategy, "1d", start_date, end_date)
    
    # 5. 更新数据示例
    print("\n【步骤5】更新数据示例")
    print("-" * 60)
    print("（取消注释以下代码以执行数据更新）")
    # framework.update_data(stock_id, "1d")
